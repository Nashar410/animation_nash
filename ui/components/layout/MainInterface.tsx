// ui/components/layout/MainInterface.tsx
...
// Utiliser le type de retour de useAppEngine pour typer les props
type AppEngine = ReturnType<typeof useAppEngine>;

interface MainInterfaceProps extends AppEngine {
  modelViewerRef: RefObject<ModelViewerRef>;
}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="p-6">
              {activeTab === 'model' ? (
                <div className="space-y-6">
                  ...
                  <div className="p-4 bg-white/5 rounded-xl border border-white/10"><p className="text-sm text-purple-200 mb-2"><strong>Model Info:</strong> {model.meshes.length} meshes, {model.animations.length} animations</p><button onClick={() => setModel(null)} className="text-sm text-red-300 hover:text-red-200 transition-colors">← Load different model</button></div>
                  {model.animations.length > 0 && (<div className="mt-6"><AnimationSelector animations={availableAnimations} currentAnimation={currentAnimation} isPlaying={isAnimationPlaying} onAnimationSelect={handleAnimationSelect} onPlayPause={handlePlayPause} onReset={handleAnimationReset} /></div>)}
                </div>
        ...
        <div className="lg:col-span-1">
          <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 shadow-2xl border border-white/20">
            ...
            <div className="rounded-xl overflow-hidden border border-white/20"><ModelViewer3D ref={modelViewerRef} model={model} camera={camera} width={400} height={400} showHelpers={showGrid} onCameraChange={setCamera} /></div>
