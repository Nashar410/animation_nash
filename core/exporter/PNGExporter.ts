...
export class PNGExporter extends Exporter {
    ...
    async export(
        ...
        this.logger.info(`Exporting ${frames.length} frames as PNG`);
        ...
        const finalCanvas = this.applyScale(canvas, settings.scale);

        // Convert to blob
        const blob = await this.canvasToBlob(finalCanvas, 'image/png');
    ...
    private applyScale(canvas: HTMLCanvasElement, scale: number): HTMLCanvasElement {
        ...
        // Use nearest neighbor for pixel art scaling
